...
public class HeavyAttack extends Move {
    public HeavyAttack() {
        super("Heavy Attack", "Deals more damage, but less likely to hit", 5);
    }
    ...
    public void onPerform(Character self, Character target) {
        ...
        System.out.println(self.name+" throws heavy attack!");

        Random random = new Random();

        // Modifies stats for heavy attack
        float newAgi = self.agi - ((float) self.str*.6f);
        float newStr = self.str + ((float) self.str*.5f);

        // Calculates hit chance
        float hitRate = .5f + (newAgi / (newAgi+target.agi)) * .5f;
        if (random.nextFloat()*100 <= hitRate*100) {
            float critMultiplier = 1.0f;

            // Calculates critical hit chance
            float critChance = (float) self.agi / 20; // Uses original agility value instead of reduced one
            if (random.nextFloat()*100 <= critChance*100) critMultiplier = 1.5f;

            // Calculates damage based on stat
            int damage = (int) ((newStr*critMultiplier)*4 - target.def*2);
            target.hp -= damage;
